...
if (InitialPDFsType == "0"):
    ...
    # plot gluon
    subplt[0].plot(x, InitialPDFgluonSAL(x, Parameters[8], Parameters[1], Parameters[2]), label="Apfel++ result")
    if showSALInitialPDFs:
        subplt[0].plot(x, InitialPDFgluonSAL(x, SALParameters[8], SALParameters[1], SALParameters[2]), label="SAL result")
    subplt[0].set_title("gluon InitialPDF")
    subplt[0].tick_params('x', labelbottom=True)
    subplt[0].legend(loc="upper right")

    # plot down quark
    subplt[2].plot(x, InitialPDFdownSAL(x, Parameters[3], Parameters[4], Parameters[5], Parameters[6], Parameters[7]), label="Apfel++ result")
    if showSALInitialPDFs:
        subplt[2].plot(x, InitialPDFdownSAL(x, SALParameters[3], SALParameters[4], SALParameters[5], SALParameters[6], SALParameters[7]), label="SAL result")
    subplt[2].set_title("down quark InitialPDF")
    subplt[2].tick_params('x', labelbottom=True)
    subplt[2].legend(loc="upper right")

    # plot up quark
    subplt[3].plot(x, InitialPDFupSAL(x, Parameters[3], Parameters[4], Parameters[5], Parameters[6], Parameters[7]), label="Apfel++ result")
    if showSALInitialPDFs:
        subplt[3].plot(x, InitialPDFupSAL(x, SALParameters[3], SALParameters[4], SALParameters[5], SALParameters[6], SALParameters[7]), label="SAL result")
    subplt[3].set_title("up quark InitialPDF")
    subplt[3].tick_params('x', labelbottom=True)
    subplt[3].legend(loc="upper right")

    # plot strange quark
    subplt[1].plot(x, InitialPDFstrangeSAL(x, Parameters[0], Parameters[3], Parameters[4], Parameters[5], Parameters[6], Parameters[7]), label="Apfel++ result")
    if showSALInitialPDFs:
        subplt[1].plot(x, InitialPDFstrangeSAL(x, SALParameters[0], SALParameters[3], SALParameters[4], SALParameters[5], SALParameters[6], SALParameters[7]), label="SAL result")
    subplt[1].set_title("strange quark InitialPDF")
    subplt[1].tick_params('x', labelbottom=True)
