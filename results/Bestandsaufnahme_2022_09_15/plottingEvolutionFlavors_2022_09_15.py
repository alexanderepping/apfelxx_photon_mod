...
pto="HO"
OutputFileName = "allData"+pto+"_2022_09_15.txt"
range_particles = 6
c = 1.75
...
num_x_vals = int(np.loadtxt(open(filepath+"data_GRV"+pto+"Sqrt2.txt"), delimiter=",", unpack=True, skiprows=3, max_rows=1))
...
for particle in range(range_particles):
    # GRVXX data at Q=sqrt(2)
    x[particle], GRVEvolved[particle],  GRVExact[particle], ratio[particle] = np.loadtxt(open(filepath+"data_GRV"+pto+"Sqrt2.txt"),    delimiter=",", unpack=True, skiprows=(particle * num_x_vals + 5), max_rows=num_x_vals)
    # SAL3XXEvolved at Q=sqrt(2)
    x[particle], SAL3Evolved[particle], SAL[particle],      ratio[particle] = np.loadtxt(open(filepath+"data_SAL3"+pto+"Evolved.txt"), delimiter=",", unpack=True, skiprows=(particle * num_x_vals + 5), max_rows=num_x_vals)
    # SAL5XXEvolved at Q=sqrt(2)
    x[particle], SAL5Evolved[particle], SAL[particle],      ratio[particle] = np.loadtxt(open(filepath+"data_SAL5"+pto+"Evolved.txt"), delimiter=",", unpack=True, skiprows=(particle * num_x_vals + 5), max_rows=num_x_vals)
...
for particle in range(range_particles):
    ...
    if (particle != 5):
        subplt[particle].plot(x[particle], GRVEvolved[particle],  label="GRV"+pto+", taken at 1.3 GeV,\nevolved to sqrt(2) GeV")
        subplt[particle].plot(x[particle], GRVExact[particle],    label="GRV"+pto+", taken at sqrt(2) GeV")
        subplt[particle].plot(x[particle], SAL[particle],         label="SALHO at sqrt(2) GeV")
        subplt[particle].plot(x[particle], SAL3Evolved[particle], label="calculated InitialPDF SAL3"+pto+",\nevolved from 1.3 GeV to sqrt(2) GeV")
        subplt[particle].plot(x[particle], SAL5Evolved[particle], label="calculated InitialPDF SAL5"+pto+",\nevolved from 1.3 GeV to sqrt(2) GeV")
...
plt.savefig(filepath+"plotAllData"+pto+"atSqrt2.pdf", bbox_inches='tight')
plt.savefig(filepath+"plotAllData"+pto+"atSqrt2.png", bbox_inches='tight')
plt.show()